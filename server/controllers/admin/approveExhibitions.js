...
module.exports = {
  approveExhibitions: async (req, res) => {
    ...
    if (userInfo.user_type === 3) {
      ...
      res.status(200).json({ message: "exhibition successfully approved" });

      if (isNull(await exhibition.findOne({ where: { id } }))) {
        res.status(404).json({ message: "exhibition not exist" });
      } else {
        await exhibition.update({ status: 1 }, { where: { id } });
      }
    } else {
